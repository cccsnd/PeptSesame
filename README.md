...
- Motif-compatible sORF candidate resource: 2,292–6,311 candidates per sesame
  ...
  independent MS-validated peptides (search-space coverage, not recall)
...
Source code, pipeline configuration, and workflow scripts are available under
the MIT license. A stable release is archived on Zenodo (https://doi.org/10.5281/zenodo.22064668).

## Reproducibility

- **Environment**: `environment.yml` defines the exact conda environment
  (Python 3.11 + pinned dependency ranges). Alternatively `pip install -e .`
  after installing the system dependencies listed in `pyproject.toml`.
- **Smoke test**: `python tests/smoke_test.py` exercises the Layer 1-3 core
  pipeline end-to-end on a tiny synthetic genome (fast, no external data
  required). Layer 4 requires the full 18-genome SSP library and is exercised
  by the real-data reruns recorded in the manuscript.
- **Checksums**: `SHA256SUMS` lists SHA-256 hashes of all pipeline scripts,
  configuration, and tests at release time. Verify with
  `sha256sum -c SHA256SUMS`.
- **Random seeds**: empirical null analyses use fixed seeds (see
  `null_model_motif.py --seed`, default 7; five independent sample batches for
  the published permutation null).
- **Workflow entry points**: `run_layer1.py <species> <genome.fa> [genes.gff]`,
  `run_layer2.py <species>`, `run_layer3.py <species>`; outputs land under
  `<PEPTSESAME_ROOT>/results/` (default: repository root) and can be redirected
  with `--outdir` where supported. BLASTP-based steps require `blastp` on PATH.
- **Data**: the 18-genome assembly list, the curated motif library, and all
  intermediate/final tables are deposited with the manuscript (Zenodo data
  record; DOI in Data Availability) and mirrored in `results/` after a
  real-data rerun.
